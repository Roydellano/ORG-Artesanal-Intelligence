...
def public_settings() -> dict:
    try:
        ...
        return {key: value for key, value in config.items() if key != "api_key"} | {
            "configured": bool(config["api_key"]), "synthetic_only": config["model"].endswith(":free"),
...
def chat(messages: list[dict[str, str]], *, max_tokens: int = 2048, timeout: float = 60,
    ...
    if model.endswith(":free") and not synthetic:
        raise OpenRouterError("Free endpoints are enabled only for app-generated fictional demos. For uploaded records use offline review or a model with no-collection/ZDR routing.")
    ...
    request = Request(
        ...
        data=json.dumps({
            ...
            "provider": ({"allow_fallbacks": False, "require_parameters": True} if synthetic else
